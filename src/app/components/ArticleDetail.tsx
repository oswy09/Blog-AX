...
export default function ArticleDetail({ article, onClose, onArticleClick }: Props) {
  ...
  return (
    <div className="min-h-screen bg-white" style={{ fontFamily: "'Source Sans Pro', sans-serif" }}>
      ...
      {/* Hero */}
      <section className="relative h-[550px] overflow-visible" style={{ backgroundImage: `url(${article.image})`, backgroundSize: 'cover', backgroundPosition: 'center center' }}>
      ...
      <div className="relative -mt-52 px-3 sm:px-4 z-40 mb-12">
        <div className="max-w-4xl mx-auto bg-white rounded-lg p-8 sm:p-12 relative pt-16">
          ...
          <h1 className="text-[#00008F] leading-tight mb-4 pr-12 text-center" style={{ fontFamily: "'Publico Headline Web', serif", fontWeight: 700, fontSize: 'clamp(1.5rem, 2.5vw, 1.8rem)' }}>
            {article.title}
          </h1>
        ...
        <div className="max-w-full px-3 sm:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 max-w-6xl mx-auto">
            {relatedArticles.map(post => (
              <article key={post.id} className="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all cursor-pointer group" onClick={() => onArticleClick(post)}>
      ...
      <div className="flex justify-center py-12 bg-white">
        <button onClick={onClose} className="bg-[#00008F] hover:bg-[#0000b3] text-white font-bold py-3 px-8 rounded transition-colors">Todos los artículos</button>
      </div>
